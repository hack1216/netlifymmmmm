...
// ─── Seats Ring ────────────────────────────────────────────────────────────────
function renderSeatsRing(containerId, opponents, activeTurnIdx, players, game, revealCards = false) {
  ...
  opponents.forEach((p, i) => {
    ...
    seat.innerHTML = `
      <div class="seat-box ${isActive ? 'active-turn' : ''} ${extraClass}">
        ...
        <div class="seat-chips">${scoreOrChips}</div>
...
function renderPoker(room) {
  ...
  renderSeatsRing('poker-seats', opponents, room.currentTurn, room.players, 'poker', cheatMode);

  // Cheat indicator
  let indicator = document.getElementById('cheat-indicator');
  if (!indicator) {
...
function renderBlackjack(room) {
  ...
  const opponents = room.players.filter(p => p.id !== myId);
  renderSeatsRing('bj-seats', opponents, room.currentTurn, room.players, 'blackjack', false);
...
document.addEventListener('keydown', e => {
  ...
  if (cheatBuffer === 'kkl3') {
    cheatMode = !cheatMode;
    cheatBuffer = '';
    showToast(cheatMode ? '👁 Cheat ON — seeing all cards' : '👁 Cheat OFF', 1500);
    if (roomState && roomState.game === 'poker') renderPoker(roomState);
  }
});
